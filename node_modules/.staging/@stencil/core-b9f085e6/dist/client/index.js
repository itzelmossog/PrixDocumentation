...
const supportsShadowDom = (buildConditionals.BUILD.shadowDom) ? !!doc.documentElement.attachShadow : false;
...
const loadModule = (cmpMeta, hostRef, hmrVersionId) => {
    ...
    /* webpackMode: "lazy" */
    `./${bundleId}.entry.js${buildConditionals.BUILD.hotModuleReplacement && hmrVersionId ? '?s-hmr=' + hmrVersionId : ''}`)).then(importedModule => importedModule[cmpMeta.$tagName$.replace(/-/g, '_')], consoleError);
};

const styles = new Map();
const cssVarShim = buildConditionals.BUILD.cssVarShim ? /*@__PURE__*/ win.__stencil_cssshim : false;
...
const patchEsm = () => {
    ...
    if (!(win.CSS && win.CSS.supports && win.CSS.supports('color', 'var(--c)'))) {
        // @ts-ignore
        return Promise.resolve(require('./css-shim-673dd43d.js'));
    }
...
const patchBrowser = async () => {
    ...
    if (importMeta !== '') {
        ...
        const scriptElm = Array.from(doc.querySelectorAll('script')).find(s => (s.src.includes(`/${buildConditionals.NAMESPACE}.esm.js`) ||
            s.getAttribute('data-namespace') === buildConditionals.NAMESPACE));
        const resourcesUrl = new URL('.', new URL(scriptElm.getAttribute('data-resources-url') || scriptElm.src, win.location.href));
        patchDynamicImport(resourcesUrl.href);
        if (!window.customElements) {
            // @ts-ignore
            await Promise.resolve(require('./dom-d4621515.js'));
        }
...
const attachStyles = (elm, cmpMeta, mode) => {
    const styleId = addStyle((buildConditionals.BUILD.shadowDom && supportsShadowDom && elm.shadowRoot)
        ? elm.shadowRoot
...
const h = (nodeName, vnodeData, ...children) => {
    ...
    walk(children);
    if (buildConditionals.BUILD.vdomAttribute && vnodeData) {
...
const setAccessor = (elm, memberName, oldValue, newValue, isSvg, flags) => {
    ...
    if (buildConditionals.BUILD.vdomClass && memberName === 'class' && !isSvg) {
        // Class
    ...
    else if (buildConditionals.BUILD.vdomListener && memberName.startsWith('on') && !isMemberInElement(elm, memberName)) {
        // Event Handlers
         ...
         */
        const isXlinkNs = buildConditionals.BUILD.svg && isSvg && (memberName !== (memberName = memberName.replace(/^xlink\:?/, ''))) ? true : false;
...
const createElm = (oldParentVNode, newParentVNode, childIndex, parentElm) => {
    ...
    if (isDef(newVNode.$text$)) {
        ...
        newVNode.$elm$ = (buildConditionals.BUILD.isDebug || buildConditionals.BUILD.hydrateServerSide) ? doc.createComment(`slot-reference:${hostTagName}`) : doc.createTextNode('');
    }
    else {
        // create element
        elm = newVNode.$elm$ = ((buildConditionals.BUILD.svg && (isSvgMode || newVNode.$tag$ === 'svg'))
        ...
        if (buildConditionals.BUILD.svg && newVNode.$tag$ === 'svg') {
...
const addVnodes = (parentElm, before, parentVNode, vnodes, startIdx, endIdx) => {
    ...
    let childNode;
    if (buildConditionals.BUILD.shadowDom && containerElm.shadowRoot && toLowerCase(containerElm.tagName) === hostTagName) {
...
const patch = (oldVNode, newVNode) => {
    ...
    if (!isDef(newVNode.$text$)) {
        ...
        if (buildConditionals.BUILD.updatable && isDef(oldChildren) && isDef(newChildren)) {
            // looks like there's child vnodes for both the old and new vnodes
            updateChildren(elm, oldChildren, newVNode, newChildren);
        }
        else if (isDef(newChildren)) {
            // no old child vnodes, but there are new child vnodes to add
            if (buildConditionals.BUILD.updatable && buildConditionals.BUILD.vdomText && isDef(oldVNode.$text$)) {
        ...
        else if (buildConditionals.BUILD.updatable && isDef(oldChildren)) {
...
const callNodeRefs = (vNode, isDestroy) => {
    if (buildConditionals.BUILD.vdomRef && vNode) {
...
const renderVdom = (hostElm, hostRef, cmpMeta, renderFnResults) => {
    ...
    renderFnResults.$elm$ = oldVNode.$elm$ = (buildConditionals.BUILD.shadowDom ? hostElm.shadowRoot || hostElm : hostElm);
...
const scheduleUpdate = async (elm, hostRef, cmpMeta, isInitialLoad) => {
    ...
    if (buildConditionals.BUILD.taskQueue) {
...
const updateComponent = (elm, hostRef, cmpMeta, instance, isInitialLoad) => {
    ...
    if (buildConditionals.BUILD.lifecycle && buildConditionals.BUILD.lazyLoad) {
        elm['s-lr'] = false;
    }
    ...
    if (buildConditionals.BUILD.lifecycle && buildConditionals.BUILD.lazyLoad && elm['s-rc'].length > 0) {
...
const postUpdateComponent = (elm, hostRef, ancestorsActivelyLoadingChildren) => {
    if ((buildConditionals.BUILD.lazyLoad || buildConditionals.BUILD.lifecycle || buildConditionals.BUILD.lifecycleDOMEvents) && !elm['s-al']) {
        const instance = buildConditionals.BUILD.lazyLoad ? hostRef.$lazyInstance$ : elm;
        ...
        if (!(hostRef.$flags$ & 512 /* hasLoadedComponent */)) {
            ...
            if (buildConditionals.BUILD.lazyLoad && buildConditionals.BUILD.cssAnnotations) {
                // DOM WRITE!
            ...
            if (buildConditionals.BUILD.lazyLoad && !ancestorComponent) {
                // on appload
        ...
        if (buildConditionals.BUILD.lifecycle && buildConditionals.BUILD.lazyLoad && ancestorComponent) {
...
const setValue = (ref, propName, newVal, cmpMeta) => {
    ...
    if (newVal !== oldVal && (!buildConditionals.BUILD.lazyLoad || !(flags & 8 /* isConstructingInstance */) || oldVal === undefined)) {
        ...
        if (!buildConditionals.BUILD.lazyLoad || hostRef.$lazyInstance$) {
            // get an array of method names of watch functions to call
            if (buildConditionals.BUILD.watchCallback && cmpMeta.$watchers$ &&
                ...
                if (watchMethods) {
                    ...
                    watchMethods.forEach(watchMethodName => {
                        try {
                            ...
                            (buildConditionals.BUILD.lazyLoad ? hostRef.$lazyInstance$ : elm)[watchMethodName].call((buildConditionals.BUILD.lazyLoad ? hostRef.$lazyInstance$ : elm), newVal, oldVal, propName);
                        }
                        catch (e) {
                            consoleError(e);
                        }
            ...
            if (buildConditionals.BUILD.updatable && (flags & (4 /* isActiveRender */ | 2 /* hasRendered */ | 16 /* isQueuedForUpdate */)) === 2 /* hasRendered */) {
...
const proxyComponent = (Cstr, cmpMeta, flags) => {
    if (buildConditionals.BUILD.member && cmpMeta.$members$) {
        ...
        members.forEach(([memberName, [memberFlags]]) => {
            if ((buildConditionals.BUILD.prop && (memberFlags & 31 /* Prop */)) || (buildConditionals.BUILD.state && (!buildConditionals.BUILD.lazyLoad || flags & 2 /* proxyState */) && (memberFlags & 32 /* State */))) {
                // proxyComponent - prop
...
const initializeComponent = async (elm, hostRef, cmpMeta, hmrVersionId, Cstr) => {
    // initializeComponent
    if ((buildConditionals.BUILD.lazyLoad || buildConditionals.BUILD.style) && (hostRef.$flags$ & 256 /* hasInitializedComponent */) === 0) {
        ...
        if (buildConditionals.BUILD.style && !Cstr.isStyleRegistered && Cstr.style) {
            ...
            if (!buildConditionals.BUILD.hydrateServerSide && buildConditionals.BUILD.shadowDom && cmpMeta.$flags$ & 8 /* needsShadowDomShim */) {
                style = await Promise.resolve(require('./shadow-css-73f95896.js')).then(m => m.scopeCss(style, scopeId, false));
            }
    ...
    const ancestorComponent = hostRef.$ancestorComponent$;
    if (buildConditionals.BUILD.lifecycle && buildConditionals.BUILD.lazyLoad && ancestorComponent && !ancestorComponent['s-lr'] && ancestorComponent['s-rc']) {
        ...
        if (!(hostRef.$flags$ & 1 /* hasConnected */)) {
            ...
            if (buildConditionals.BUILD.lifecycle && buildConditionals.BUILD.lazyLoad) {
                ...
                while ((ancestorComponent = (ancestorComponent.parentNode || ancestorComponent.host))) {
                    ...
                    if ((ancestorComponent['s-init'] && !ancestorComponent['s-lr']) || (buildConditionals.BUILD.hydrateClientSide && ancestorComponent.nodeType === 1 /* ElementNode */ && ancestorComponent.hasAttribute('s-id'))) {
            ...
            if (buildConditionals.BUILD.taskQueue && buildConditionals.BUILD.mode) {
...
const setContentReference = (elm, contentRefElm) => {
    ...
    // content was first placed, which is useful later on
    let crName;
    if (buildConditionals.BUILD.isDebug) {
    ...
    lazyBundles.forEach(lazyBundle => lazyBundle[1].forEach(compactMeta => {
        ...
        const HostElement = class extends HTMLElement {
            ...
            constructor(self) {
                ...
                super(self);
                self = this;
                if (buildConditionals.BUILD.lifecycle) {
                    this['s-lr'] = false;
                    this['s-rc'] = [];
                }
                registerHost(self);
...
const createEvent = (ref, name, flags) => {
    ...
    return {
        emit: (detail) => elm.dispatchEvent(new (buildConditionals.BUILD.hydrateServerSide ? win.CustomEvent : CustomEvent)(name, {
...
const getContext = (_elm, context) => {
    if (context in Context) {
        ...
        return !!buildConditionals.BUILD.hydrateServerSide;
    }

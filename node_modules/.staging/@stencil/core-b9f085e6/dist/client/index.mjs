import { BUILD, NAMESPACE } from '@stencil/core/build-conditionals';
...
const patchEsm = () => {
    ...
    if (!(win.CSS && win.CSS.supports && win.CSS.supports('color', 'var(--c)'))) {
        // @ts-ignore
        return import('./css-shim-f7ddb189.js');
    }
    ...
    if ((BUILD.lazyLoad || BUILD.style) && (hostRef.$flags$ & 256 /* hasInitializedComponent */) === 0) {
        ...
        if (BUILD.style && !Cstr.isStyleRegistered && Cstr.style) {
            ...
            if (!BUILD.hydrateServerSide && BUILD.shadowDom && cmpMeta.$flags$ & 8 /* needsShadowDomShim */) {
                style = await import('./shadow-css-984bac74.js').then(m => m.scopeCss(style, scopeId, false));
            }
...
export { Build, Context, H, H as HTMLElement, Host, attachShadow, bootstrapLazy, connectedCallback, consoleError, createEvent, cssVarShim, disconnectedCallback, doc, getAssetPath, getConnect, getContext, getElement, getHostRef, getMode, getValue, h, insertVdomAnnotations, isMemberInElement, loadModule, nextTick, parsePropertyValue, patchBrowser, patchDynamicImport, patchEsm, plt, postUpdateComponent, proxyComponent, proxyNative, readTask, registerHost, registerInstance, renderVdom, setMode, setValue, styles, supportsConstructibleStylesheets, supportsListenerOptions, supportsShadowDom, win, writeTask };

...
function getBuildConditionals(buildCtx) {
    const b = {
        shadow: false,
        slot: false,
        svg: false,
...
async function generateBuildStats(config, compilerCtx, buildCtx, buildResults) {
...
async function generateStatsOutputTarget(config, compilerCtx, buildCtx, buildResults, outputTarget) {
    try {
        let jsonData;
...
async function initFsWatcher(config, compilerCtx, buildCtx) {
...
async function buildFinish(config, compilerCtx, buildCtx, aborted) {
...
function logHmr(logger, buildCtx) {
    // this is a rebuild, and we've got hmr data
    // and this build hasn't been aborted
...
function cleanupUpdateMsg(logger, msg, fileNames) {
...
class BuildContext {
    ...
    async finish() {
        const results = await buildFinish(this.config, this.compilerCtx, this, false);
        const buildLog = {
            buildId: this.buildId,
            messages: this.buildMessages.slice(),
            progress: 1
...
function getProgress(completedTasks) {
    let progressIndex = 0;
    ...
    return {
        ...
        resolveId(importee, importer) {
            ...
            if (importee.endsWith('.json')) {
                ...
                if (collectionDir) {
                    return config.sys.path.resolve(config.sys.path.dirname(importer).replace(collectionDir, config.srcDir), importee);
                }
            }
            return null;
        },
...
const COMPILER_BUILD = {
    id: '20190608135224',
    minfyJs: 'terser4.0.0_3',
    optimizeCss: 'autoprefixer9.6.0_cssnano4.1.10_postcss7.0.17_3',
    transpiler: 'typescript3.5.1_3'
};
        ...
        if (importNode.moduleSpecifier.text === '@stencil/core') {
            if (importNode.importClause && importNode.importClause.namedBindings && importNode.importClause.namedBindings.kind === ts.SyntaxKind.NamedImports) {
                ...
                if (keepImports.length > 0) {
                    return ts.updateImportDeclaration(importNode, undefined, undefined, ts.createImportClause(undefined, ts.createNamedImports(keepImports.map(name => ts.createImportSpecifier(undefined, ts.createIdentifier(name))))), importNode.moduleSpecifier);
                }
            }
            return null;
        }
...
const LegacyScriptTarget = ts.ScriptTarget.ES5;
const ScriptTarget = ts.ScriptTarget.ES2017;
...
function createImportDeclaration(importPath, importFnName, importAs = null) {
    return ts.createImportDeclaration(undefined, undefined, ts.createImportClause(undefined, ts.createNamedImports([
        ts.createImportSpecifier(typeof importFnName === 'string' && importFnName !== importAs ? ts.createIdentifier(importFnName) : undefined, ts.createIdentifier(importAs))
    ...
    if (module === ts.ModuleKind.CommonJS) {
        ...
        const requires = importFnNames.map(importKey => {
            ...
            return ts.createVariableStatement(undefined, ts.createVariableDeclarationList([
                ts.createVariableDeclaration(varName, undefined, ts.createPropertyAccess(ts.createCall(ts.createIdentifier('require'), [], [ts.createLiteral(importPath)]), ts.createIdentifier(importName)))
            ]));
        });
...
const CONNECTED_CALLBACK = '__stencil_connectedCallback';
const H = '__stencil_h';
...
function addLazyImports(transformCtx, compilerCtx, tsSourceFile) {
    const importFns = [
        ...COMMON_IMPORTS,
...
function replaceStylePlaceholders(cmps, modeName, code) {
    cmps.forEach(cmp => {
...
function getStyleTextPlaceholder(cmp) {
    return `STYLE_TEXT_PLACEHOLDER:${cmp.tagName}`;
}

function addComponentStylePlaceholders(classMembers, cmp) {
    if (!cmp.hasStyle) {
        return;
    }
...
function addLazyElementGetter(classMembers, cmp) {
    // @Element() element;
    // is transformed into:
...
function removeStaticMetaProperties(classNode) {
...
function computeFlags(eventMeta) {
    let flags = 0;
...
function getStatement(propName, method, arg) {
...
function updateLazyComponentConstructor(classMembers, cmp) {
...
function addWatchers(classMembers, cmp) {
...
function transformHostData(classElements) {
...
function syntheticRender(hasRender) {
    const hArguments = [
        // __stencil_Host
...
function updateLazyComponentClass(opts, classNode, cmp) {
    return ts.updateClassDeclaration(classNode, classNode.decorators, classNode.modifiers, classNode.name, classNode.typeParameters, classNode.heritageClauses, updateLazyComponentMembers(opts, classNode, cmp));
}
function updateLazyComponentMembers(opts, classNode, cmp) {
...
function transformToLazyComponentText(compilerCtx, buildCtx, opts, cmp, inputText) {
    let outputText = null;
    try {
        const transpileOpts = {
            compilerOptions: {
...
function lazyComponentTransform(compilerCtx, opts) {
    return transformCtx => {
        return tsSourceFile => {
...
async function updateToLazyComponent(config, compilerCtx, buildCtx, cmp) {
...
function createComponentExport(cmp, lazyModuleFilePath) {
...
function addNativeConnectedCallback(classMembers, cmp) {
    ...
    if (cmp.isPlain && cmp.hasRenderFn) {
        ...
        const connectedCallback = classMembers.find(classMember => {
            return (ts.isMethodDeclaration(classMember) && classMember.name.escapedText === methodName);
        });
        const prependBody = [
            fnCall,
        ];
        if (connectedCallback != null) {
            ...
            const callbackMethod = ts.createMethod(undefined, undefined, undefined, methodName, undefined, undefined, undefined, undefined, ts.createBlock(prependBody, true));
...
function addNativeElementGetter(classMembers, cmp) {
    // @Element() element;
    // is transformed into:
...
function updateNativeConstructor(classMembers, cmp, ensureSuper) {
    if (cmp.isPlain) {
        return;
    }
    const cstrMethodIndex = classMembers.findIndex(m => m.kind === ts.SyntaxKind.Constructor);
...
function updateNativeComponentClass(classNode, cmp) {
    return ts.updateClassDeclaration(classNode, classNode.decorators, classNode.modifiers, classNode.name, classNode.typeParameters, updateNativeHostComponentHeritageClauses(classNode), updateNativeHostComponentMembers(classNode, cmp));
}
function updateNativeHostComponentHeritageClauses(classNode) {
...
function updateNativeHostComponentMembers(classNode, cmp) {
    const classMembers = removeStaticMetaProperties(classNode);
...
function addComponentStyle(classMembers, cmp) {
    if (!cmp.hasStyle) {
        return;
    }
...
function addNativeImports(transformCtx, compilerCtx, tsSourceFile) {
    const importFns = [
        ...COMMON_IMPORTS,
...
function transformToNativeComponentText(compilerCtx, buildCtx, cmp, inputJsText) {
    let outputText = null;
    try {
        const transpileOpts = {
            compilerOptions: {
...
function nativeComponentTransform(compilerCtx) {
    return transformCtx => {
        return tsSourceFile => {
            function visitNode(node) {
                if (ts.isClassDeclaration(node)) {
...
async function updateToNativeComponent(config, compilerCtx, buildCtx, cmp) {
    const inputFilePath = cmp.jsFilePath;
    const inputFileDir = config.sys.path.dirname(inputFilePath);
    const inputFileName = config.sys.path.basename(inputFilePath);
...
function createComponentExport$1(cmp, lazyModuleFilePath) {
    const originalClassName = cmp.componentClassName;
...
function componentEntryPlugin(config, compilerCtx, buildCtx, build, entryModules) {
...
function pluginHelper(config, compilerCtx, builtCtx) {
    return {
        name: 'pluginHelper',
        async resolveId(importee, importer) {
            if (importee) {
...
async function bundleApp(config, compilerCtx, buildCtx, build, bundleAppOptions) {
    ...
    try {
        ...
        const rollupOptions = Object.assign({}, config.rollupConfig.inputOptions, { input: bundleAppOptions.inputs, plugins: [
...
async function generateRollupOutput(build, options, config, entryModules) {
    if (build == null) {
        return null;
    }
...
async function writeLazyModule(config, compilerCtx, destinations, entryModule, shouldMinify, code, modeName, sufix) {
...
function getBundleId(config, entryKey, shouldHash, code, modeName, sufix) {
    if (shouldHash) {
...
function formatLazyBundleRuntimeMeta(bundleId, cmps) {
    return [
        bundleId,
        cmps.map(cmp => formatComponentRuntimeMeta(cmp, true))
    ];
}
function formatComponentRuntimeMeta(compilerMeta, includeMethods) {
    let flags = 0;
...
function stringifyRuntimeData(data) {
    // stringify the data, then remove property double-quotes so they can be property renamed
    return JSON.stringify(data);
}
function formatComponentRuntimeMembers(compilerMeta, includeMethods = true) {
    return Object.assign({}, formatPropertiesRuntimeMember(compilerMeta.properties), formatStatesRuntimeMember(compilerMeta.states), includeMethods ? formatMethodsRuntimeMember(compilerMeta.methods) : {});
}
function formatPropertiesRuntimeMember(properties) {
...
function formatFlags(compilerProperty) {
...
function formatAttrName(compilerProperty) {
...
function formatStatesRuntimeMember(states) {
    const runtimeMembers = {};
...
function formatMethodsRuntimeMember(methods) {
    const runtimeMembers = {};
...
function formatHostListeners(compilerMeta) {
...
function computeListenerFlags(listener) {
    let flags = 0;
    if (listener.capture) {
        flags |= 2 /* Capture */;
    }
...
async function generateLazyEntryModule(config, compilerCtx, buildCtx, rollupResult, destinations, sourceTarget, shouldMinify, isBrowserBuild, sufix) {
    ...
    const outputs = await Promise.all(entryModule.modeNames.map(modeName => writeLazyModule(config, compilerCtx, destinations, entryModule, shouldMinify, code, modeName, sufix)));
    return {
        rollupResult,
...
function sortBundleModules(a, b) {
    ...
    if (a.cmps.some(cmp => bDependants.includes(cmp.tagName)))
        return 1;
    if (b.cmps.some(cmp => aDependants.includes(cmp.tagName)))
        return -1;
    ...
    if (b.cmps.some(cmp => aDependencies.includes(cmp.tagName)))
        return 1;
    if (aDependants.length < bDependants.length)
        return -1;
    if (aDependants.length > bDependants.length)
        return 1;
...
function sortBundleComponents(a, b) {
    ...
    if (a.directDependants.includes(b.tagName))
        return 1;
    if (b.directDependants.includes(a.tagName))
        return -1;
    if (a.directDependencies.includes(b.tagName))
        return 1;
    if (b.directDependencies.includes(a.tagName))
        return -1;
    if (a.dependants.includes(b.tagName))
        return 1;
    if (b.dependants.includes(a.tagName))
        return -1;
    if (a.dependencies.includes(b.tagName))
        return 1;
    if (b.dependencies.includes(a.tagName))
        return -1;
    if (a.dependants.length < b.dependants.length)
        return -1;
    if (a.dependants.length > b.dependants.length)
        return 1;
...
async function generateEsmBrowser(config, compilerCtx, buildCtx, build, rollupBuild, outputTargets) {
    const esmOutputs = outputTargets.filter(o => !!o.esmDir && !!o.isBrowserBuild);
...
async function generateSystem(config, compilerCtx, buildCtx, build, rollupBuild, outputTargets) {
    if (!config.buildEs5) {
        return;
    }
...
async function generateCjs(config, compilerCtx, buildCtx, _build, rollupBuild, outputTargets) {
    if (!config.buildDist) {
        return;
    }
...
async function performCopyTasks(config, compilerCtx, buildCtx, copyTasks) {
    try {
...
async function waitForCopyTasks(buildCtx) {
    await Promise.all(buildCtx.pendingCopyTasks);
    buildCtx.pendingCopyTasks.length = 0;
}

async function processCopyTasks(config, dest, tasks) {
    if (!Array.isArray(tasks)) {
        return [];
    }
    return flatOne(await Promise.all(tasks.map(task => processCopyTask(config, dest, task))));
}
async function processCopyTask(config, dest, copyTask) {
    if (!copyTask.src) {
...
function processSimpleTask(config, copyTask, destAbsDir) {
    return {
...
async function processGlobTask(config, copyTask, dest) {
    const globOpts = {
        cwd: config.srcDir,
        nodir: true
    };
...
async function copyAssets(config, compilerCtx, buildCtx, outputTargets) {
    const allCopyTasks = flatOne(await Promise.all(outputTargets.map(async (o) => [
...
async function outputCollections(config, compilerCtx, buildCtx) {
    ...
    copyAssets$1(config, compilerCtx, buildCtx, outputTargets);
...
async function copyAssets$1(config, compilerCtx, buildCtx, outputTargets) {
...
async function bundleHydrateApp(config, compilerCtx, buildCtx, build, appEntryCode) {
    try {
        const treeshake = !config.devMode && config.rollupConfig.inputOptions.treeshake !== false;
        const rollupOptions = Object.assign({}, config.rollupConfig.inputOptions, { input: '@app-entry', inlineDynamicImports: true, plugins: [
                loaderPlugin({
...
function addHydrateImports(transformCtx, compilerCtx, tsSourceFile) {
    const importFns = [
        ...COMMON_IMPORTS,
...
function addHydrateRuntimeCmpMeta(classMembers, cmp) {
...
function updateHydrateComponentClass(classNode, cmp) {
    return ts.updateClassDeclaration(classNode, classNode.decorators, classNode.modifiers, classNode.name, classNode.typeParameters, classNode.heritageClauses, updateHydrateHostComponentMembers(classNode, cmp));
}
function updateHydrateHostComponentMembers(classNode, cmp) {
    const classMembers = removeStaticMetaProperties(classNode);
    updateLazyComponentConstructor(classMembers, cmp);
    addLazyElementGetter(classMembers, cmp);
    addWatchers(classMembers, cmp);
...
function transformToHydrateComponentText(compilerCtx, buildCtx, cmp, inputJsText) {
    let outputText = null;
    try {
        const transpileOpts = {
            compilerOptions: {
...
function hydrateComponentTransform(compilerCtx) {
    return transformCtx => {
        return tsSourceFile => {
            function visitNode(node) {
                if (ts.isClassDeclaration(node)) {
...
async function getHydratePackageJson(config, compilerCtx, hydrateAppFilePath, hydrateDtsFilePath) {
    let pkgName;
    try {
...
async function copyHydrateRunner(config, compilerCtx, hydrateAppDirPath) {
...
function generateEntryPoint(entryModules) {
    let count = 0;
    const result = [
    ...
    entryModules.forEach(entry => entry.cmps.forEach(cmp => {
        if (cmp.isPlain) {
            result.push(`export { ${dashToPascalCase(cmp.tagName)} } from '${entry.entryKey}';`);
        }
        else {
            const meta = stringifyRuntimeData(formatComponentRuntimeMeta(cmp, false));
            const exportName = dashToPascalCase(cmp.tagName);
            result.push(`import { ${exportName} as $Cmp${count} } from '${entry.entryKey}';`, `export const ${exportName} = /*@__PURE__*/proxyNative($Cmp${count}, ${meta});`);
            count++;
        }
    }));
    return result.join('\n');
...
const COMPONENTS_DTS_HEADER = `/* tslint:disable */
/**
...
async function generateWww(config, compilerCtx, buildCtx, criticalPath, outputTarget) {
    ...
    performCopyTasks(config, compilerCtx, buildCtx, await processCopyTasks(config, outputTarget.appDir, outputTarget.copy));
    if (!config.buildEs5) {
...
async function generateJsonDocs(compilerCtx, docsData, outputTargets) {
...
function updateLastStyleComponetInputs(config, compilerCtx, buildCtx) {
    if (config.watch) {
...
function getStyleId(cmp, modeName, isScopedStyles) {
...
function requiresScopedStyles(encapsulation, commentOriginalSelector) {
...
function computeDependants(cmps) {
    cmps.forEach(cmp => {
...
function resolveTransitiveDependants(cmp, cmps) {
    cmp.dependants = cmps
...
function getModule(config, compilerCtx, sourceFilePath) {
...
function resetModule(moduleFile) {
...
function gatherVdomMeta(m, args) {
    ...
    if (args.length > 1) {
        if (ts.isObjectLiteralExpression(args[1])) {
            ...
            const propsWithText = props
                .filter(p => p.name && p.name.text && p.name.text.length > 0)
                .map(p => p.name.text);
...
function parseCallExpression(m, node) {
...
function visitCallExpressionArgs(m, callExpressionName, args) {
...
function visitCallExpressionArg(m, arg) {
...
function parseImport(config, compilerCtx, buildCtx, moduleFile, dirPath, importNode) {
...
function parseStaticMethods(staticMembers) {
...
function parseStaticListeners(staticMembers) {
...
function setComponentBuildConditionals(cmpMeta) {
...
function parseClassMethods(cmpNode, cmpMeta) {
...
function parseStaticElementRef(staticMembers) {
...
function parseStaticEncapsulation(staticMembers) {
...
function parseStaticEvents(staticMembers) {
...
function parseStaticProps(staticMembers) {
...
function parseStaticStates(staticMembers) {
...
function parseStaticWatchers(staticMembers) {
...
function normalizeStyles(config, tagName, componentFilePath, styles) {
    styles.forEach(style => {
        if (style.modeName === DEFAULT_STYLE_MODE) {
...
function normalizeExternalStyle(config, componentFilePath, externalStyle) {
...
function parseStaticStyles(config, compilerCtx, tagName, componentFilePath, isCollectionDependency, staticMembers) {
    const styles = [];
    let parsedStyleStr = getStaticValue(staticMembers, 'styles');
...
function parseStringLiteral(m, node) {
...
function parseStaticComponentMeta(config, compilerCtx, transformCtx, typeChecker, cmpNode, moduleFile, nodeMap, transformOpts) {
    ...
    function visitComponentChildNode(node) {
    ...
    if (transformOpts.addCompilerMeta) {
        ...
        cmpNode = ts.updateClassDeclaration(cmpNode, cmpNode.decorators, cmpNode.modifiers, cmpNode.name, cmpNode.typeParameters, cmpNode.heritageClauses, classMembers);
    }
    // add to module map
...
function parseVirtualProps(docs) {
    return docs.tags
        .filter(({ name }) => name === 'virtualProp')
        .map(parseVirtualProp)
        .filter(prop => !!prop);
}
function parseVirtualProp(tag) {
...
function parseAssetsDirs(config, staticMembers, componentFilePath) {
...
function convertStaticToMeta(config, compilerCtx, buildCtx, typeChecker, collection, transformOpts) {
    return transformCtx => {
        let dirPath;
        let moduleFile;
        function visitNode(node) {
...
function parseCollectionManifest(config, compilerCtx, buildCtx, collectionName, collectionDir, collectionJsonStr) {
...
function parseCollectionDependencies(collectionManifest) {
    return (collectionManifest.collections || []).map(c => c.name);
}
function parseGlobal(config, compilerCtx, collectionDir, collectionManifest) {
...
function parseBundles(collectionManifest) {
...
function invalidArrayData$1(arr) {
    return (!arr || !Array.isArray(arr) || arr.length === 0);
}

function parseCollection(config, compilerCtx, buildCtx, pkgJsonFilePath, pkgData) {
...
function addExternalImport(config, compilerCtx, buildCtx, moduleFile, resolveFromDir, moduleId) {
...
function validatePublicName(config, diagnostics, memberName, decorator, memberType, node) {
...
function methodDecoratorsToStatic(config, diagnostics, sourceFile, decoratedProps, typeChecker, newMembers) {
...
function parseMethodDecorator(config, diagnostics, sourceFile, typeChecker, method) {
    ...
    const methodMeta = {
        complexType: {
            ...
            references: Object.assign({}, getAttributeTypeInfo(returnTypeNode, sourceFile), getAttributeTypeInfo(method, sourceFile)),
            return: returnString
        },
        docs: {
...
function isTypePromise(typeStr) {
    return /^Promise<.+>$/.test(typeStr);
}

function componentDecoratorToStatic(config, diagnostics, cmpNode, newMembers, componentDecorator) {
...
function validateComponent(config, diagnostics, componentOptions, cmpNode, componentDecorator) {
...
function findTagNode(propName, node) {
...
function normalizeExtension(config, styleUrls) {
...
function elementDecoratorsToStatic(diagnostics, decoratedMembers, typeChecker, newMembers) {
...
function parseElementDecorator(_diagnostics, _typeChecker, prop) {
...
function eventDecoratorsToStatic(config, diagnostics, decoratedProps, typeChecker, newMembers) {
...
function parseEventDecorator(config, diagnostics, typeChecker, prop) {
...
function getEventName$1(eventOptions, memberName) {
...
function getComplexType(typeChecker, node) {
    const sourceFile = node.getSourceFile();
...
function validateEventEmitterMemberName(config, diagnostics, node, memberName) {
...
function listenDecoratorsToStatic(config, diagnostics, decoratedMembers, newMembers) {
...
function parseListenDecorators(config, diagnostics, method) {
...
function parseListener(config, diagnostics, eventName, opts = {}, methodName, decoratorNode) {
...
function isValidTargetValue(prefix) {
...
function propDecoratorsToStatic(config, diagnostics, decoratedProps, typeChecker, newMembers) {
    const connect = [];
    const context = [];
...
function parsePropDecorator(config, diagnostics, typeChecker, prop, context, connect, newMembers) {
...
function getAttributeName(_diagnostics, propName, propOptions) {
...
function getReflect(_diagnostics, propOptions) {
...
function getComplexType$1(typeChecker, node, type) {
...
function removeProp(prop, classElements) {
...
function stateDecoratorsToStatic(diagnostics, _sourceFile, decoratedProps, typeChecker, newMembers) {
...
function stateDecoratorToStatic(_diagnostics, _typeChecker, prop) {
...
function watchDecoratorsToStatic(diagnostics, decoratedProps, newMembers) {
...
function parseWatchDecorator(_diagnostics, method) {
    const methodName = method.name.getText();
...
function convertDecoratorsToStatic(config, diagnostics, typeChecker) {
    return transformCtx => {
        function visit(tsSourceFile, node) {
            if (ts.isClassDeclaration(node)) {
...
function visitClass(config, diagnostics, typeChecker, tsSourceFile, cmpNode) {
...
function removeStencilDecorators(classMembers) {
...
async function transpileService(config, compilerCtx, buildCtx) {
    let changedTsFiles;
...
async function buildTsService(config, compilerCtx, buildCtx) {
    const transpileCtx = {
...
async function transpileTsFile(config, services, ctx, sourceFilePath, checkCacheKey, useFsCache) {
...
async function outputFile(ctx, outputFilePath, outputText) {
...
async function scanDirForTsFiles(config, compilerCtx, buildCtx) {
...
function createConfigKey(config, compilerOptions) {
...
async function validateTypesMain(config, compilerCtx, buildCtx) {
...
function getCleanRollupConfig(rollupConfig) {
    ...
    if (rollupConfig.inputOptions && isObject(rollupConfig.inputOptions)) {
        cleanRollupConfig = Object.assign({}, cleanRollupConfig, { inputOptions: pluck(rollupConfig.inputOptions, ['context', 'moduleContext', 'treeshake']) });
    }
...
function validateResourcesUrl(resourcesUrl) {
